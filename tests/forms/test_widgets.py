...
class FormDividerWidgetTests(AdditionalAsserts, TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.DummyForm = DummyForm
        cls.safe_trim = lambda v: v.strip() if isinstance(v, str) else v
    ...
    def switch_rendering_tests(
        ...
        self.assertEqual(
            self.safe_trim(switch_icon_element.attrs.get('aria-label')),
            label_show[lang].strip() if collapsed else label_hide[lang].strip()
        )
        self.assertEqual(
            self.safe_trim(switch_icon_element.attrs.get('data-aria-label-inactive')),
